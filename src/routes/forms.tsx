...
import { Plus, Link2, Copy, ArrowLeft, FileSpreadsheet, Trash2, Edit2, CheckCircle2 } from "lucide-react";

export const Route = createFileRoute("/forms")({ component: FormsPage });

function FormsPage() {
  const { can, forms, setForms } = useApp();
  if (!can("forms")) return <AppShell><AccessDenied moduleName="Forms & Data" /></AppShell>;
    ...
    <AppShell>
      <PageContainer>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {forms.map((f) => (
            <Card key={f.id} className="p-5 hover:border-navy cursor-pointer transition-colors" onClick={() => setSelected(f.id)}>
              <div className="flex items-start justify-between mb-3">
                <div className="h-10 w-10 rounded-lg bg-navy/10 text-navy grid place-items-center">
                  <FileSpreadsheet className="h-5 w-5" />
                </div>
                <Badge variant="secondary" className="bg-muted border-0 text-xs">{f.submissions.length} submissions</Badge>
              </div>
              <h3 className="font-semibold">{f.name}</h3>
              <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{f.description}</p>
              <div className="mt-4 flex items-center gap-2 text-xs">
                <Link2 className="h-3.5 w-3.5 text-muted-foreground" />
                <code className="text-muted-foreground font-mono truncate">ets.church/f/{f.shareId}</code>
              </div>
            </Card>
          ))}
        </div>
          ...
          <Card className="w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button className="bg-navy text-navy-foreground" onClick={handleCreateForm}>Create Form</Button>
              </div>
            </div>
          </Card>
        </div>
...
function FormDetail({ 
  ...
  const [newFieldType, setNewFieldType] = useState<"text" | "number" | "date" | "select" | "checkbox" | "file">("text");
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div>
            ...
            <div className="mt-3 flex items-center gap-2">
              <Input readOnly value={`${window.location.origin}/f/${form.shareId}`} className="w-[340px] font-mono text-xs" />
          ...
          <div className="flex items-center gap-2">
            <div className="flex rounded-md border overflow-hidden">
              <button onClick={()=>setTab("data")} className={`px-4 py-2 text-sm ${tab==="data" ? "bg-navy text-navy-foreground" : "hover:bg-muted"}`}>Data</button>
              <button onClick={()=>setTab("builder")} className={`px-4 py-2 text-sm ${tab==="builder" ? "bg-navy text-navy-foreground" : "hover:bg-muted"}`}>Builder</button>
            </div>
          </div>
        </div>
                  ...
                  <table className="w-full text-sm min-w-[500px]">
                    <thead>
                      <tr className="bg-muted/50 text-left text-xs uppercase tracking-wider text-muted-foreground">
                        ...
                        {form.fields.map((f) => (
                          <th key={f.id} className="px-4 py-3 font-medium whitespace-nowrap">{f.label}</th>
                        ))}
                      </tr>
                    </thead>
              ...
              {form.fields.map((f) => (
                <div key={f.id} className="flex items-center justify-between border rounded-lg p-3">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{f.type}{f.options ? ` • ${f.options.length} options` : ""}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className="bg-muted border-0 uppercase text-[10px] tracking-wider">{f.type}</Badge>
                    <Button 
                      variant="outline" 
                      size="sm" 
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="outline" onClick={() => setIsAddFieldModalOpen(false)}>Cancel</Button>
                <Button className="bg-navy text-navy-foreground" onClick={handleAddField}>Add Field</Button>
              </div>
            </div>
          </Card>
        </div>
