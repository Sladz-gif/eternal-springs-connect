...
function FinancePage() {
  ...
  if (!anyFinance)
    return (
      <AppShell>
        <AccessDenied moduleName="Finance" />
      </AppShell>
    );
  ...
  const defaultTab = can("finance.giving")
    ? "giving"
    ...
    <AppShell>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button size="sm" className="bg-navy text-navy-foreground hover:bg-navy/90">
                    ...
                    New entry <ChevronDown className="h-4 w-4 ml-2" />
                  </Button>
                </DropdownMenuTrigger>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <Card className="p-5">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              {givingList.length} contributions logged
            </div>
          </Card>
          <Card className="p-5">
            <div className="flex items-center justify-between">
          ...
          <Card className="p-5">
            ...
            <div className="text-2xl font-semibold mt-2">
              {formatCurrency(fundsList.reduce((s, f) => s + f.balance, 0))}
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              Across {fundsList.length} funds
            </div>
          </Card>
          <Card className="p-5">
            <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              {formatCurrency(
                projectsList.filter((p) => p.status === "Active").reduce((s, p) => s + p.raised, 0),
              )}{" "}
              raised
            </div>
          </Card>
                    ...
                    <tbody className="divide-y">
                      {givingList
                        .filter(
                          (g) =>
                            !q ||
                            g.receipt.toLowerCase().includes(q.toLowerCase()) ||
                            g.memberName.toLowerCase().includes(q.toLowerCase()),
                        )
                        .map((g) => (
                          <tr key={g.id} className="hover:bg-muted/30">
                            ...
                            <td className="px-4 py-3">
                              {fundsList.find((f) => f.id === g.fundId)?.name}
                            </td>
                            <td className="px-4 py-3">
                              <Badge
                                variant="secondary"
                                className={
                                  g.type === "Recurring"
                                    ? "bg-sky/15 text-sky-foreground border-0"
                                    : "bg-muted border-0"
                                }
                              >
                                {g.type}
                ...
                <div className="p-4 border-b flex flex-wrap gap-3 items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Approval workflow — pending items require a Finance Lead.
                    </p>
                  </div>
                  <Button
                    size="sm"
                      ...
                      {expensesList.map((e) => (
                        <tr key={e.id} className="hover:bg-muted/30">
                          ...
                          <td className="px-4 py-3">
                            ...
                            {e.receipt && (
                              <div className="text-xs text-muted-foreground">📎 {e.receipt}</div>
                            )}
                          </td>
                            ...
                            <Badge
                              className={
                                e.status === "Approved"
                                  ...
                                  : e.status === "Pending"
                                    ? "bg-gold text-gold-foreground border-0"
                        ...
                        <div key={f.id}>
                          ...
                          <div className="h-2 bg-muted rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${pct}%`,
                                background: FUND_COLORS[i % FUND_COLORS.length],
                              }}
                            />
                          </div>
                        </div>
                    ...
                    <Card key={p.id} className="p-5 flex flex-col">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <Badge
                          className={
                            p.status === "Active"
                              ? "bg-sky text-sky-foreground border-0"
                      ...
                      <div className="mt-4">
                        ...
                        <div className="mt-1.5 text-xs text-muted-foreground flex justify-between">
                          <span>
                            {pct}% funded • {p.contributors} contributors
                          </span>
                      ...
                      {[
                        ...
                        return (
                          <tr key={r.d}>
                            ...
                            <td
                              className={`py-2.5 text-right font-medium ${v >= 0 ? "text-success" : "text-destructive"}`}
                            >
                              {v >= 0 ? "+" : ""}
                              {formatCurrency(v)}
                            </td>
      ...
      >
        <GivingForm
          ...
          onCancel={() => setIsGivingModalOpen(false)}
          members={membersList}
          funds={fundsList}
        />
      </Modal>
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button
          ...
          onClick={() =>
            onSubmit({ department, category, description, date, amount: Number(amount) })
          }
        >
          Submit Expense
        </Button>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button className="bg-navy text-navy-foreground" onClick={() => onSubmit({ name })}>
          Add Fund
        </Button>
      </div>
    </div>
...
function ProjectForm({
  ...
  onSubmit: (data: { name: string; description: string; goal: number; deadline: string }) => void;
  onCancel: () => void;
}) {
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button
          className="bg-navy text-navy-foreground"
          onClick={() => onSubmit({ name, description, goal: Number(goal), deadline })}
        >
          Add Project
        </Button>
      </div>
