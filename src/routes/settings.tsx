    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Extensible — new roles are instantly assignable with user accounts.
            </p>
          </div>
          <Button
            size="sm"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                        {modules
                          ...
                          .map((m) => (
                            <div
                              key={m.id}
                              className="flex items-center justify-between border rounded-md px-3 py-2 bg-surface"
                            >
                              <label className="flex items-center gap-2 text-sm">
                                <Checkbox
                                  checked={selected.includes(m.id)}
                                  onCheckedChange={() => toggle(selected, m.id, setSelected)}
                                />
                                {m.name}
                              </label>
                              {selected.includes(m.id) && (
                                <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                  <Checkbox
                                    checked={editable.includes(m.id)}
                                    onCheckedChange={() => toggle(editable, m.id, setEditable)}
                                  />
                                  Edit
                                </label>
                              )}
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  addRole({
                    ...
                    fullAccess,
                    modules: fullAccess ? modules.map((m) => m.id) : selected,
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {r.fullAccess && (
                        <Badge className="bg-gold text-gold-foreground border-0">Full access</Badge>
                      )}
                    </div>
                        ...
                        r.modules.map((mid) => {
                          ...
                          return (
                            <Badge
                              key={mid}
                              variant="secondary"
                              className={`border-0 text-xs ${editable ? "bg-navy text-navy-foreground" : "bg-muted"}`}
                            >
                              {m?.name}
                              {editable ? "" : " · view"}
                            </Badge>
      ...
      <Card className="p-5">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Add new modules or departments to make them assignable to roles.
        </p>
        ...
        <div className="space-y-2 border-t pt-3">
          <Input
            placeholder="Module or department name"
            value={newModName}
          ...
          <Select value={newModGroup} onValueChange={setNewModGroup}>
            ...
            <SelectContent>
              {["Overview", "People", "Finance", "Operations", "Admin"].map((g) => (
