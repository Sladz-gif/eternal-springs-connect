...
import { totals, events as evts, projects, formatCurrency } from "@/lib/mock-data";
        ...
        <div className="rounded-2xl bg-navy text-navy-foreground p-6 lg:p-8 mb-6 flex flex-wrap items-center gap-6 justify-between">
          <div className="min-w-0">
            ...
            <p className="text-sm text-navy-foreground/75 mt-2 max-w-xl">
              Here's what's happening across the congregation this week — attendance is trending up
              and two fundraising projects are within reach of their goals.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          ...
          <div className="cursor-pointer" onClick={() => navigate({ to: "/attendance" })}>
            <Kpi
              ...
              value={String(totals.attendance)}
              delta="+3.1%"
              tone="sky"
            />
          </div>
          <div className="cursor-pointer" onClick={() => navigate({ to: "/finance" })}>
            <Kpi
              icon={Wallet}
              label="Giving (Month)"
              value={formatCurrency(totals.givingThisMonth)}
              delta="+8.7%"
              tone="gold"
            />
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          <Card className="p-5 lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <div className="cursor-pointer" onClick={() => navigate({ to: "/attendance" })}>
                <h3 className="font-semibold hover:text-navy transition-colors">
                  Attendance trend
                </h3>
                <p className="text-xs text-muted-foreground">Last 7 Sundays</p>
              </div>
              <Badge variant="secondary" className="bg-sky/15 text-sky-foreground border-0">
                Weekly
              </Badge>
            </div>
            <div className="h-64">
          ...
          <Card className="p-5">
            ...
            <ul className="divide-y">
              {evts.slice(0, 5).map((e) => (
                <li
                  key={e.id}
                  className="py-3 flex items-center gap-4 cursor-pointer hover:bg-muted/30 rounded-lg px-2 -mx-2"
              ...
              {projects
                ...
                .map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="cursor-pointer hover:bg-muted/30 rounded-lg p-2 -m-2"
