...
const SERVICE_TYPES = ["Sunday Service", "Mid Week on Thursday", "Prayers on Friday"] as const;
...
function AttendancePage() {
  ...
  if (!can("members"))
    return (
      <AppShell>
        <AccessDenied moduleName="Attendance" />
      </AppShell>
    );
  ...
  const currentRecord = attendanceRecords.find(
    (r) => r.date === selectedDate && r.serviceType === selectedService,
  );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          ...
          <Card className="p-5 sm:col-span-2 lg:col-span-1">
            ...
            <div className="text-2xl font-semibold">
              {presentCount} <span className="text-sm text-muted-foreground">/ {totalCount}</span>
            </div>
            <div className="mt-2 h-2 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-navy" style={{ width: `${attendancePercent}%` }} />
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              {attendancePercent}% Present | {markedCount} marked
            </div>
          </Card>
        </div>
