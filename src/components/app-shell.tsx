...
function Shell({ children }: { children: ReactNode }) {
  ...
  const { currentRole, roles, setCurrentRoleId, currentUser, signOut, notifications, markNotificationAsRead, clearNotifications } = useApp();
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full h-full">
        <header className="h-16 border-b bg-surface flex items-center gap-3 px-4 sticky top-0 z-20 shrink-0">
          ...
          <div className="relative flex-1 min-w-0">
            ...
            <Input placeholder="Search members, funds, projects, forms…" className="pl-9 h-10 bg-background w-full" />
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="h-9 w-9 grid place-items-center rounded-md hover:bg-muted relative flex-shrink-0" aria-label="Notifications">
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((notif) => (
                    <DropdownMenuItem key={notif.id} onClick={() => !notif.read && markNotificationAsRead(notif.id)}>
                      <div className="flex items-start gap-3 py-1 w-full">
                        <div className={`h-2 w-2 rounded-full mt-2 flex-shrink-0 ${!notif.read ? 'bg-gold' : 'bg-muted'}`} />
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-2 h-9 pl-2 pr-3 rounded-md hover:bg-muted flex-shrink-0">
              ...
              <div className="hidden sm:block text-left">
                <div className="text-xs font-medium leading-tight">{currentUser?.name || 'User'}</div>
                <div className="text-[11px] text-muted-foreground leading-tight">{currentRole.name}</div>
              </div>
    ...
    <PageContainer>
      <div className="border rounded-xl bg-surface p-10 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Your role does not include access to <span className="font-medium text-foreground">{moduleName}</span>.
          <br />Ask a Super Admin to grant permission.
        </p>
      </div>
    </PageContainer>
  );
